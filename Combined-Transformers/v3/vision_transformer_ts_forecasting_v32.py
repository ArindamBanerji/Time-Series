...
def prepare_data(data: pd.DataFrame, sequence_length: int = 20):
    ...
    # Ensure the sequence length is a perfect square
    perfect_square_length = int(np.ceil(np.sqrt(sequence_length))**2)
    if perfect_square_length != sequence_length:
        pad_length = perfect_square_length - sequence_length
        X = np.pad(X, ((0, 0), (0, pad_length), (0, 0)), mode='constant')
    
    sqrt_seq_len = int(np.sqrt(perfect_square_length))
    X = X.transpose(0, 2, 1)
